...
export default function ProjectDetail() {
  ...
  if (!project) {
    return <div className="pt-32 text-center text-xl text-white bg-[#0B0F19] min-h-screen">Project not found</div>;
  }

  return (
    <main className="pt-28 bg-[#0B0F19] text-white">
      <section className="max-w-6xl mx-auto px-6 py-20">
        <h1 className="text-5xl font-semibold mb-6">{project.title}</h1>
        <p className="text-gray-400 text-lg mb-6">Industry: {project.industry}</p>
        <p className="text-gray-300 leading-relaxed mb-10">{project.overview}</p>
      </section>

      <section className="bg-[#0B0F19] py-20">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-3xl font-semibold mb-8 text-white">Technologies Used</h2>
          <ul className="grid md:grid-cols-2 gap-4">
            {project.technologies.map((tech, i) => (
              <li key={i} className="bg-[rgba(255,255,255,0.05)] backdrop-blur-sm border border-[#1F2937] p-4 rounded-lg text-gray-300">
                {tech}
              </li>
            ))}
          </ul>
      ...
      <section className="py-20">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-3xl font-semibold mb-8 text-white">Project Results</h2>
          <ul className="space-y-4">
            {project.results.map((r, i) => (
              <li key={i} className="border border-[#1F2937] bg-[rgba(255,255,255,0.05)] p-4 rounded-lg text-gray-300">
                {r}
              </li>
            ))}
          </ul>
